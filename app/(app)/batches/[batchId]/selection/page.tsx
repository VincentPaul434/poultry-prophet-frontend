...
function ReportContent({ payload }: { payload: SelectionReviewPayload }) {
  ...
  return <div className="space-y-4">
    ...
    {payload.finance && <ReportSection title="Recorded batch finance"><div className="grid grid-cols-3 gap-2"><Fact label="Income" value={`₱${Number(payload.finance.recordedIncome).toLocaleString()}`} detail="Posted records" /><Fact label="Expense" value={`₱${Number(payload.finance.recordedExpense).toLocaleString()}`} detail="Posted records" /><Fact label="Net cash flow" value={`₱${Number(payload.finance.recordedNetCashFlow).toLocaleString()}`} detail="Not accounting profit" /></div><p className="mt-3 text-xs text-muted-foreground">{payload.finance.limitation}</p></ReportSection>}
